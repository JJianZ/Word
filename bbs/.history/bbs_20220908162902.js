...
app.post('/register',(req,res,next) => {   // 判断用户名,邮箱是否存在 ,两次密码是不是一致
  ...
  if (reg.test(regInfo.name)) {
    try {
      db.prepare('INSERT INTO users (name,password,email) VALUES(?,?,?)').run(user.name,user.password,user.email)
...
app.post('/login',(req,res,next) => {   // 判断是否登录成功
  ...
  var target = users.find(it => it.name == loginInfo.name && it.password == md5(loginInfo.password))  // 在users库中匹配用户名和密码

  loginInfo.password = md5(loginInfo.password)

  var target = db.prepare('SELECT * FROM USERS WHERE name = $name AND password = $password').get(loginInfo.name,loginInfo.password)
...
app.delete('/comment/:commentId',(req,res,next) => {  // 删除评论
  res.type('html')
  if (req.signedCookies.loginName) {//评论是自己发的
