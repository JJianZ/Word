...
app.use(function(req,res,next) {  // 根据已经登录了的信息查出用户
  if (req.loginUser) {
    req.loginUser = db.prepare('SELECT rowid as userId,name,email FROM users WHERE name = ?').get(req.loginUser)
  } else {
    req.loginUser = null
  }
...
app.get('/', (req,res,next) => {  // get首页
  ...
  var posts = db.prepare(`
    SELECT posts.rowid as postId, title,content,createdAt,userId,name FROM posts
    JOIN users
...
app.post('/login',(req,res,next) => {   // 判断是否登录成功
  ...
  var returnUrl = req.get('referer') ?? '/'    // req的query可以读到url中的querystring 地址栏login/ 后面的内容
...
app.delete('/post/:id',(req,res,next) => {   // 删帖
  ...
  if (req.loginUser) {
    ...
    if (post) {
      if (post.owner == req.loginUser) {
        var idx = posts.findIndex(it => it.Id == req.params.id)  // 找到第一个满足的下标
        posts.splice(idx,1)
...
app.delete('/comment/:commentId',(req,res,next) => {  // 删除评论
  res.type('html')
  if (req.loginUser) {//评论是自己发的
    var comment = comments.find(it => it.Id == req.params.commentId)
    if (comment) {
      if (comment.owner == req.loginUser) {
        var idx = comments.findIndex(it => it.Id == req.params.id)  // 找到第一个满足的下标
        comments.splice(idx,1)
        fs.writeFileSync('./comments.json',JSON.stringify(comments,null,2))
        res.end('ok')
      } else { // 评论是发在自己的帖子下面
        var post = posts.find(it => it.id == comment.postId)
        if (post.owner == req.loginUser) {
          var idx = comments.findIndex(it => it.Id == req.params.id)  // 找到第一个满足的下标
          comments.splice(idx,1)
          fs.writeFileSync('./comments.json',JSON.stringify(comments,null,2))
