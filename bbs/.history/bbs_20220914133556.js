...
app.post('/register',(req,res,next) => {   // 判断用户名,邮箱是否存在 ,两次密码是不是一致
  ...
  const form = formidable({
    multiples: true,
    uploadDir: './uploads',  // 上传地址
    keepExtensions: true,
    maxFileSize: 2 * 1024 * 1024,   // 上传最大文件尺寸 2M
  });
  ...
  form.parse(req, (err, fields, files) => {
    ...
    var activiteId = uuid().slice(0, 8)  // 创建一个传输过去id

    var user = {     // 存储用户信息
      name: regInfo.name,
      email: regInfo.email,
      password: md5(regInfo.password),
      avatar: files.avatar.newFilename,  // 文件名
    ...
    if (reg.test(regInfo.name)) {
      try {
        db.prepare('INSERT INTO users (name,password,email,avatar,activited) VALUES ($name,$password,$email,$avatar,$activited)').run(user)
      } catch (e) {
    ...
    res.end('注册成功，请至邮箱点击激活链接<a href="/resend" target="_blank">重新发送</a>' + '   ' + `<a href=${activiteLink}>点此激活</a>`)
    } else {
...
app.get('/user/:userId', (req, res, next) => {  // 用户主页
  res.type('html')
  ...
  if (user) {
    // 发过的帖子
    var posts = db.prepare(`
      SELECT posts.content,posts.userId, posts.createdAt,postId,title,avatar
      FROM posts
      JOIN users
      ON posts.userId = users.rowid
      JOIN posts
      ON posts.postId = posts.rowid
      WHERE posts.userId = ?
    `).all(userId)
...
app.get('/add-post', (req, res, next) => {  // 发帖页
  res.type('html')
...
var forgetPasswordMap = {    // 每个链接映射到每个用户
  'asdqasdasdwt': {name:'username', password: 'passsd', email: ''}
}
app.get('/forget-password', (req, res, next) => {  // 找回密码界面
